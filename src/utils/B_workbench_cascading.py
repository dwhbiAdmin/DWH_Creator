...
class ColumnCascadingEngine:
    ...
    def regenerate_all_columns_with_unique_ids(self, include_technical_fields: bool = True) -> bool:
        ...
        try:
            ...
            # Clear existing columns using proper column structure from config
            columns_config = self.config_manager.get_columns_sheet_config()
            empty_columns_df = pd.DataFrame(columns=columns_config['headers'])
    ...
    def _load_data_type_mappings(self) -> pd.DataFrame:
        ...
        try:
            # Check if DataTypeMappings sheet exists before trying to read it
            available_sheets = self.excel_utils.get_sheet_names(self.config_path)
            if "DataTypeMappings" not in available_sheets:
                self.logger.info("DataTypeMappings sheet not found, using defaults")
    ...
    def _load_technical_columns_config(self) -> Dict:
        ...
        try:
            ...
            self.logger.error(f"Failed to load technical columns config: {str(e)}")
            self.logger.info("Using default technical columns configuration")
            return self._create_default_technical_columns()
    ...
    def _generate_columns_by_relation_type(self, target_artifact: pd.Series, upstream_columns: pd.DataFrame,
        ...
        for processed_col in processed_columns:
            ...
            column_group = self._determine_column_group(
                processed_col['column_name'], 
                processed_col.get('is_primary_key', False),
                converted_type
            )
    ...
    def _generate_ai_column_content(self, column_name: str, artifact_name: str, data_type: str) -> Dict[str, str]:
        ...
        Args:
            column_name: Name of the column
            artifact_name: Name of the parent artifact  
            data_type: Data type of the column
            
        Returns:
            Dict with 'comment' and 'business_name' keys
        """
        try:
            ...
            if hasattr(self, 'workbench_manager') and self.workbench_manager.is_ai_available():
                ai_workbench = self.workbench_manager.ai_workbench
                ...
                return {
                    'comment': comment or f"Data field for {column_name.replace('_', ' ').title()}",
                    'business_name': business_name or column_name.replace('_', ' ').title()
                }
            else:
                # Fallback to basic generation if AI not available
                return {
                    'comment': f"Data field for {column_name.replace('_', ' ').title()}",
                    'business_name': column_name.replace('_', ' ').title()
                }
                
        except Exception as e:
            self.logger.warning(f"AI content generation failed for {column_name}: {e}")
            return {
                'comment': f"Data field for {column_name.replace('_', ' ').title()}",
                'business_name': column_name.replace('_', ' ').title()
            }

    def _determine_column_group(self, column_name: str, is_primary_key: bool, data_type: str) -> str:
        """
        Determine the appropriate column group based on column characteristics.
        ...
        Returns:
            String indicating the column group
        """
        column_name_lower = column_name.lower()
        
        # Primary key identification
        if is_primary_key or column_name_lower.endswith('_id') or column_name_lower.endswith('id'):
            return 'primary_key'
        
        # Technical fields identification
        if (column_name_lower.startswith('__') or 
            column_name_lower.startswith('_') or
            column_name_lower in ['created_date', 'modified_date', 'load_date', 'source_system', 
                                 'source_file', 'row_hash', 'is_active', 'effective_date', 
                                 'end_date', 'is_current']):
            return 'technical_fields'
        
        # Foreign key identification  
        if '_id' in column_name_lower and not column_name_lower.endswith('_id'):
            return 'foreign_key'
        ...
        # Measure/fact fields (numeric types)
        if data_type.upper() in ['INT', 'BIGINT', 'DECIMAL', 'FLOAT', 'NUMERIC', 'MONEY']:
            if any(keyword in column_name_lower for keyword in ['amount', 'total', 'sum', 'count', 'qty', 'quantity', 'price', 'cost', 'value']):
                return 'measures'
        
        # Default to attributes for descriptive fields
        return 'attributes'
